...
void rec(int ind, vector<bool> visited, vector<bool> isBig, stack<int> s, bool jocker){
    if(ind == en){
        ans++;
        return;
    }
    for(int i = 0; i < grid[ind].size(); i++){
        ...
        if(newNode == sn) continue;
        if(!visited[newNode]){
            if(!isBig[newNode])
                visited[newNode] = true;
            s.push(newNode);
            rec(newNode, visited, isBig, s, jocker);
            if(!isBig[newNode])
                visited[newNode] = false;
            s.pop();
        }else{
            if(jocker && !isBig[newNode]){
                s.push(newNode);
                rec(newNode, visited, isBig, s, false);
                s.pop();
            }
        }
    }
